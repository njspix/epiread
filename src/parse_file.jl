...
function get_feature_pos_in_read(rle::String; mode::Char)
	J = Vector{Int64}()
	V = Vector{Int}()
	counter = 0
...
function get_feature_pos_absolute(rle::String, read_start::Int64; mode::Char)
	J, V, error = get_feature_pos_in_read(rle, mode = mode)
...
function analyze_read(read::EPIREAD.Record, master_I::Vector{Int64}, master_J::Vector{Int64}, master_V::Vector{Int}; mode::Char)
	# println("analyzing read $(EPIREAD.name(read))/$(EPIREAD.readnum(read)) ($(EPIREAD.chrom(read))): $(EPIREAD.cg_rle(read))")
	J, V, error = get_feature_pos_absolute(EPIREAD.cg_rle(read), EPIREAD.chromstart(read), mode = mode)
...
function analyze_read(rle::String, chromstart::Int64, read_name::String, master_I::Vector{Int64}, master_J::Vector{Int64}, master_V::Vector{Int}, indel_error_reads::Vector{String}; mode::Char)
	# println("analyzing read $(EPIREAD.name(read))/$(EPIREAD.readnum(read)) ($(EPIREAD.chrom(read))): $(EPIREAD.cg_rle(read))")
	J, V, error = get_feature_pos_absolute(rle, chromstart, mode = mode)
...
function analyze_record_pair(read1::EPIREAD.Record, read2::EPIREAD.Record, master_I::Vector{Int64}, master_J::Vector{Int64}, master_V::Vector{Int}, indel_error_reads::Vector{String}; mode::Char)
	if !allsame(EPIREAD.chrom.((read1, read2)))
		for each in (read1, read2)
			analyze_read(each, master_I, master_J, master_V, mode = mode)
		end
	else 
		if !are_overlapped(read1, read2)
			...
			analyze_read(rle, chromstart, EPIREAD.name(read1), master_I, master_J, master_V, indel_error_reads, mode = mode)
		end 
	end
end

function analyze_chr(reader::EPIREAD.Reader, max_isize::Int, leftover_record::EPIREAD.Record; mode::Char)
    ...
    while !eof(reader)
        ...
        if (last_chr ≠ "" && last_chr ≠ current_chr)
            ...
            for each in record_cache
                analyze_read(each, I, J, V, mode = mode)
            end
            ...
            if name in cached_names 
                ...
                analyze_record_pair(record, pair_record, I, J, V, indel_error_reads, mode = mode)
...
function parse_infile(filename::String; max_isize = 1000, mode::Char)
    ...
    while !eof(reader)
        (sparse_results, new_indel_errors, last_chr, leftover_record) = analyze_chr(reader, max_isize, leftover_record, mode = mode)
